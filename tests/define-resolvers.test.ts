...
const { defineQuery, defineMutation, createResolvers } = defineResolvers<Resolvers>()

test("defineQuery", () => {
  const resolvers = defineQuery("whoami", (parent, args, context, _info) => {
...
test("defineMutation", () => {
  const resolvers = defineMutation("login", (parent, args, _context, _info) => {
...
test("createResolvers", () => {
  ...
  resolvers.defineQuery("whoami", (parent, args, context, _info) => {
    return context.userId ? { id: context.userId, name: "test" } : null
  })

  resolvers.defineMutation("login", (parent, args, _context, _info) => {
    return { id: 1, name: args.username }
  })
